...
- All requests/response payloads will use the `application/json` MIME type.

- Response status codes

    - 200 = success on `GET`, `PATCH`, `PUT`, and some `POST` (as describess below)
    ...
    - 401 = unauthorized (no bearer token present in the request headers), or if the bearer token can't be varified
    ...
    - 415 = client sent payoad using `Content-type` other than `application/json`
...
Defines a new canvas using the authenticated user as the owner, and with specified dimensions and
background color.
